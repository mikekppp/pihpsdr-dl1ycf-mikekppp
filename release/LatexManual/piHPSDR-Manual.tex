...
on LINUX (see e.g. Fig. \ref{fig:zoom}).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{What is a spectrum?}
Some people get confused if the spectrum shows a noise floor at $-122$ dBm, while their S-meter
reading ,,without signal'' is $-109$ dBm. These people then often ask which of the two figures tells
...
that is the frequency width of one pixel on the screen. An immediate consequence of this is, that the
noise floor moves down when increasing the ZOOM value, since the frequency width of one pixel gets
smaller. Another option would be to normalise the spectrum to one Hz, that is, to display the
energy within an interval one Hz wide. While it would be easy to do so, the noise floor is then
much lower of what one is used from other SDR programs and I wanted to avoid a new source of
confusion.

The S-meter reading, on the other hand, integrates the spectral energy density over the filter
passband. For example, if your spectrum is 96 kHz wide but displayed in a window 800 pixels wide,
then the width of one pixel is 120 Hz. If the spectrum shows a noise floor at -122 dBm, this means
the energy density of the noise is $-122$ dBm per 120 Hz. The S-meter reading for a filter
2400 Hz wide is then $-109$ dBm which is twenty times more energy than $-122$ dBm. Switching to
a filter 5000 Hz wide increases the S-meter reading  to $-106$ dBm (a factor of two
 is 3 dB).

If you want to measure the noise floor and report or compare the result,
the best you can do is to get an S-meter reading (in dBm) with
a filter that is 1000 Hz wide, and then subtract 30 from the S-meter value to
produce a noise floor figure in dBm/Hz.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
wide (which is 1/8 of the RX sample rate, 1/8 of 96 kHz in our
...
When changing the zoom value, then the pan value is
automatically adjusted such that the RX frequency
(the CTUN frequency if CTUN is active) is at the 
centre of the display. In the example shown, the PAN
sliders has been manually adjusted such that the signal
at the RX frequency (14100 kHz) appears close to the left
edge of the display. In my view, PAN makes little sense
if not using CTUN. While it is possible to move the PAN
slider when the zoom factor is unity, this simply has
no effect.
...
If the checkbox \rett{Server Start Stops Radio} is checked, then the radio protocol is stopped when
the server starts. In this case, the screen ,,freezes'' on the server side. This option is for automatic
remote stations where the same radio can be accessed from several computers or SDR programs, and where
it has to be avoided that \pH takes over the radio although no client is connected. The \pH
server automatically (re-) starts the protocol if a client is connected, and stops it when a
client disconnect. The protocol is also always (re-) started when the server terminates (e.g. by
un-checking the \rett{Server Enable} box).
...
Note that when the client exits and the connection is closed, the server stops the protocol. The protocol
is then restarted when a (new) client connects, or when the \rett{Restart} button is pressed on the 
server side in the main menu. The idea here is that the remote radio is released while it is not used,
even if the \pH server instance remains running. It should be possible to use the radio from another
SDR application after the client disconnects, and the remote instance of \pH (server side) can then
reconnect later if desired.
